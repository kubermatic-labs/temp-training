...
servicename="$(kubectl get serviceaccount -n default "$accountname" -o jsonpath='{.secrets[0].name}')"
token="$(kubectl get secret -n default "$servicename" -o jsonpath='{.data.token}' | base64 -d)"
